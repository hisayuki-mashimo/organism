...
Class maze
{
    ...
    {
        $this->_road_id = 'road.'.(count($this->_road_ids)+1);
        $this->squares[$target_pillar_key]  = $this->_buildSquareText('road');
        $this->_pillars[$target_pillar_key] = $this->_road_id;
    ...
    private function _getTargetPillar()
    {
        ...
        foreach($this->_pillars as $key => $value){
            if($value == 'wall') $target_pillars[$key] = true;
        }
    ...
    private function _makeWall($target_pillar_key)
    {
        ...
        if($aim = $this->_getAim($ver, $nex)){
            ...
            $this->squares[$next_wall_key] = $this->_buildSquareText('wall');
            if($this->_pillars[$next_pillar_key] == 'road'){
            //壁が四方の壁に到達していなければ、壁の形成を続ける。
                $this->_pillars[$next_pillar_key] = $this->_road_id;
                return $next_pillar_key;
            } else{
                array_push($this->_road_ids, $this->_road_id);
            }
        }
    }
    ...
    private function _getAim($ver, $nex)
    {
        ...
        $aims = array();
        if(isset($this->_pillars[$pillar_key_on])    && $this->_pillars[$pillar_key_on]    != $this->_road_id) $aims['on']    = true;
        if(isset($this->_pillars[$pillar_key_right]) && $this->_pillars[$pillar_key_right] != $this->_road_id) $aims['right'] = true;
        if(isset($this->_pillars[$pillar_key_under]) && $this->_pillars[$pillar_key_under] != $this->_road_id) $aims['under'] = true;
        if(isset($this->_pillars[$pillar_key_left])  && $this->_pillars[$pillar_key_left]  != $this->_road_id) $aims['left']  = true;
