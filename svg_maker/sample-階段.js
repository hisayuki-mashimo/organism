...
const [coordinates, surfaces] = Array(stepL * orbitL + 1).fill(null).reduce((res, _, i) => {
  ...
  res[0][`${i}OD`] = { X: XO, Y, Z: ZO };
  res[0][`${i}ID`] = { X: XI, Y, Z: ZI };
  if (i < stepL * orbitL) {
    res[0][`${i}OU`] = { X: XO, Y: Y + stepH, Z: ZO };
    res[0][`${i}IU`] = { X: XI, Y: Y + stepH, Z: ZI };
    res[1][`${i}V`] = [`${i}OD`, `${i}ID`, `${i}IU`, `${i}OU`];
  }
  if (i > 0) {
    res[1][`${i}H`] = [`${i - 1}OU`, `${i - 1}IU`, `${i}ID`, `${i}OD`];
  }
