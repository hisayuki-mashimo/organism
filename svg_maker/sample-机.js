...
const [coordinates, surfaces] = Object.keys(coordinatesBase).reduce((res, key) => {
  res[0][`${key}.LF`] = { ...coordinatesBase[key] };
  res[0][`${key}.LB`] = { ...coordinatesBase[key], Z: coordinatesBase[key].Z * -1 };
  res[0][`${key}.RF`] = { ...coordinatesBase[key], X: coordinatesBase[key].X * -1 };
  res[0][`${key}.RB`] = { ...coordinatesBase[key], X: coordinatesBase[key].X * -1, Z: coordinatesBase[key].Z * -1 };
  res[1][key] = [`${key}.LF`, `${key}.LB`, `${key}.RB`, `${key}.RF`];
