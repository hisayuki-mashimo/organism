...
class Labyrinth
{
    ...
    public function getRooms()
    {
        ...
        foreach($this->_room_tiles as $rgon_coord => $param){
            ...
            $room->top      = $edges['top'] * ($this->rgon_side + $this->mete_side) + $pdngs['top'];
            $room->lft      = $edges['lft'] * ($this->rgon_side + $this->mete_side) + $pdngs['lft'];
            $room->rgt      = $edges['rgt'] * ($this->rgon_side + $this->mete_side) + $this->rgon_side - 1 - $pdngs['rgt'];
            $room->btm      = $edges['btm'] * ($this->rgon_side + $this->mete_side) + $this->rgon_side - 1 - $pdngs['btm'];
     ...
     */
    protected function _extendRoomOut($coord)
    {
     ...
     * @param   int $coord  部屋集合データ座標
     */
    protected function _extendRoomIn($coord)
    {
     ...
     * @param   int $coord  部屋集合データ座標
     */
    protected function _reflectRoom($coord)
    {
     ...
     * @return  string          方向
     */
    protected function _getAgainstAim($aim)
    {
        switch($aim){
     ...
     * @return  array
     */
    protected function _getEnableAdvanceAims($coord, $type = 'sect')
    {
        $enable_aims = array();
