...
class Labyrinth
{
                ...
                foreach(array(($param['from'] - 1), ($param['to'] + 1)) as $n => $var){
                    ...
                    if(isset($this->_rgon_tiles[$agst_rgon_coord]) && $this->_rgon_tiles[$agst_rgon_coord]['type'] == $this->_type_room){
                        ...
                        if(! empty($this->_room_tiles[$agst_room_coord]['sect_pdngs'])){
                            ...
                            $agst_pdng_2 = $this->_room_tiles[$agst_room_coord]['sect_pdngs'][$this->_getEgoAim($agst_aim, (($n == 0) ? 'lft' : 'rgt'))];
    ...
    protected function _setGate($rgon_coord, $aim)
    {
        ...
        $this->_gate_tiles[$gate_coord] = array('room' => $room_coord);
    ...
    protected function _measureByGateRadar($base_gate_coord, $trgt_room_coord)
    {
        ...
        switch(true){
            case isset($relation[$base_aim]):
                return 5; break;
            case isset($relation[$this->_getEgoAim($base_aim)]):
                switch(true){
                    case isset($relation[$this->_getEgoAim($base_aim, 'lft')]):
                    case isset($relation[$this->_getEgoAim($base_aim, 'rgt')]):
                        return 3; break 2;
                    default:
                        return 1;
                }
    ...
    protected function _connectRgon($strt_rgon_coord, $trgt_rgon_coord)
    {
        ...
        $relation_params = array(
            'strt' => array('rgon' => $strt_rgon_coord, 'aim' => $relation_aim),
            'trgt' => array('rgon' => $trgt_rgon_coord, 'aim' => $this->_getEgoAim($relation_aim))
        );
        $relation_cond = '';
        ...
        foreach($relation_params as $role => $param){
            switch($this->_rgon_tiles[$param['rgon']]['type']){
                case $this->_type_room:
                    ...
                    $gate_coord = $param['rgon'] . ':' . $param['aim'];
                    if(! isset($this->_gate_tiles[$gate_coord])){
                        $this->_setGate($param['rgon'], $param['aim']);
                    }
                    $relation_params[$role]['coords'] = $this->_specifyGateCands($param['rgon'], $param['aim']);
                    $relation_params[$role]['type']   = $this->_type_room;
                    break;
                case $this->_type_road:
                    $relation_params[$role]['coords'] = $this->_specifyJunctionCands($param['rgon'], $param['aim']);
                    $relation_params[$role]['type']   = $this->_type_road;
            }
        }

$this->_lb('++++', $relation_params['strt']['rgon'], '<>', $relation_params['trgt']['rgon'], '++++');
        $this->_paveRoad($relation_params['strt'],  $relation_params['trgt']);
    ...
    protected function _paveRoad($strt_params, $trgt_params)
    {
        ...
        if($strt_params['type'] == $this->_type_road){ $_roads_strt = $this->_road_tiles[$strt_params['rgon']]['roads']; }
        if($trgt_params['type'] == $this->_type_road){ $_roads_trgt = $this->_road_tiles[$trgt_params['rgon']]['roads']; }
                        ...
                        try{
                            ...
                            foreach($params as $i => $param){
                                ...
                                for($n = 0; $n < $param['limit']; $n ++){
                                    if($this->_sect_tiles[$pres_coord]['type'] == $this->_type_wall){
                                        $this->_sect_tiles[$pres_coord]['type'] = $this->_type_road;
                                        $this->_sect_tiles[$pres_coord]['aim']  = $strt_params['aim'];
                                        if($strt_params['type'] == $this->_type_road){ $this->_registerToRoad($strt_params['rgon'], $pres_coord); }
                                        if($trgt_params['type'] == $this->_type_road){ $this->_registerToRoad($trgt_params['rgon'], $pres_coord); }
                                    }
                            ...
                            if($strt_params['type'] == $this->_type_road){ $this->_registerToRoad($strt_params['rgon'], $trgt_coord); }
                            if($trgt_params['type'] == $this->_type_road){ $this->_registerToRoad($trgt_params['rgon'], $trgt_coord); }
                            ...
                            if($strt_params['type'] == $this->_type_road){ $this->_road_tiles[$strt_params['rgon']]['roads'] = $_roads_strt; }
                            if($trgt_params['type'] == $this->_type_road){ $this->_road_tiles[$trgt_params['rgon']]['roads'] = $_roads_trgt; }
                            $this->_lb('H');
                        }
                    }
                    if($paved == false){
     ...
     *
     * @access  private
     * @param   string  $room_coord
     * @param   string  $trgt_aim
     */
    protected function _specifyGateCands($room_coord, $trgt_aim)
    {
        list($ver, $nex) = explode('_', $room_coord);
        $pdngs = $this->_room_tiles[$room_coord]['sect_pdngs'];
        $side  = $this->rgon_side;
        switch($trgt_aim){
            case 'top': $replacer = 'P_S'; $scdy_min = ($side * $nex) + $pdngs['lft'] + 1; $scdy_max = ($side * $nex) + $side - $pdngs['rgt'] - 2; $prmy = ($side * $ver) + $pdngs['top']; break;
            case 'lft': $replacer = 'S_P'; $scdy_min = ($side * $ver) + $pdngs['top'] + 1; $scdy_max = ($side * $ver) + $side - $pdngs['btm'] - 2; $prmy = ($side * $nex) + $pdngs['lft']; break;
            case 'rgt': $replacer = 'S_P'; $scdy_min = ($side * $ver) + $pdngs['top'] + 1; $scdy_max = ($side * $ver) + $side - $pdngs['btm'] - 2; $prmy = ($side * $nex) + $side - $pdngs['rgt'] - 1; break;
            case 'btm': $replacer = 'P_S'; $scdy_min = ($side * $nex) + $pdngs['lft'] + 1; $scdy_max = ($side * $nex) + $side - $pdngs['rgt'] - 2; $prmy = ($side * $ver) + $side - $pdngs['btm'] - 1;
        }
     ...
     * @return  string          方向
     */
    protected function _getEgoAim($aim, $relation = 'dis')
    {
        $aim_relations = array(
        ...
        foreach($this->_getAims() as $cand_aim){
            ...
            if(isset($this->_sect_tiles[$trgt_coord])){
                ...
                foreach($this->_getAims() as $adjt_aim){
                    ...
                    $coord_2 = $this->_getCoord($coord_1, $this->_getEgoAim($adjt_aim, 'rgt'));
    ...
    protected function _getZhouCoords($sect_coord, $bafang = false)
    {
        ...
        foreach($this->_getAims() as $zhou_aim){
            ...
            if(isset($this->_sect_tiles[$zhou_coord])){
                ...
                if($bafang == true){
                    $sltg_coord = $this->_getCoord($sect_coord, $this->_getEgoAim($zhou_aim, 'rgt'));
