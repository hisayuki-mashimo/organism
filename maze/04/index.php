...
class SectionsMaker
{
    ...
    public function build()
    {
        ...
        for($i = 0; $i < $this->rooms_count; $i ++){
            ...
            foreach($aim_cands as $j => $cand){
                if(($j == 0) || (rand(0, 1) == 1)){
                    $this->rooms[$coord]['gates'][$cand['road_coord']] = true;
                    $this->rooms[$coord]['roads'][$cand['road_coord']] = true;
    ...
    private function tieGate()
    {
        ...
        if(count($this->rooms) < 2){
            echo 66666; vd($this->rooms);exit;
        }
        foreach($this->rooms as $_coord => $room){
            if($_coord != $own_room_coord){
                $room_coords[] = $_coord;
            }
        }
        shuffle($room_coords);
        $room_coord = $room_coords[0];
        //$room = $this->sections[$room_coord];
        $room = $this->rooms[$room_coord];
        list($v_2, $n_2) = explode('_', $room_coord);
        $roads = array();
        $this->log .= "(${v_1}/${n_1})---(${v_2}/${n_2})";
        ...
        for($l = 0; $l < 30; $l ++){
            ...
            $gate = $this->sections[$gate_coord];
            $roads[$gate_coord] = true;

            if($gate['type'] != $this->type_wall){
                if($gate['room'] != $own_room_coord){
                    $room_coords = explode('|', $gate['room']);
                    $room_coords[] = $own_room_coord;
                    sort($room_coords);
                    $this->sections[$gate_coord]['room'] = implode('|', $room_coords);
