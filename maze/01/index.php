...
class SectionsMaker
{
    ...
    public function build()
    {
        ...
        // 部屋構成
        $coords = array_keys($this->sections_unitary);
        shuffle($coords);
    ...
    private function tieRoom()
    {
        ...
        for($i = 0; $i < 50; $i ++){
            ...
            $coords = array();
            if    ($v_1 < $v_2){ $coords[] = ($v_1 + 1) . '_' . ($n_1 - 0); }
            elseif($v_1 > $v_2){ $coords[] = ($v_1 - 1) . '_' . ($n_1 - 0); }
            if    ($n_1 < $n_2){ $coords[] = ($v_1 - 0) . '_' . ($n_1 + 1); }
            elseif($n_1 > $n_2){ $coords[] = ($v_1 - 0) . '_' . ($n_1 - 1); }
            shuffle($coords);
            $coord = $coords[0];
...
$sections_maker = new SectionsMaker(array('rooms_count' => 6));
