...
class SectionsMaker
{
    ...
    {
        $_v = ($this->ver - ($this->ver % 2)) / 2;
        $_n = ($this->nex - ($this->nex % 2)) / 2;
        $_center = "${_v}_${_n}";
        $_room_cands = array();
        ...
        // 部屋構成
        shuffle($_room_cands);
        array_splice($_room_cands, 0, 0, $_center);
        ...
        for($k = 0; $k < 100; $k ++){// 無限ループ防止
            $this->log .= '<hr />';
            //$this->log .= '[[' . implode('|', array_keys($this->gates)) . ']]<br />';
    ...
    private function tieGate()
    {
        ...
        $_coords = array();
        $_room_coords = array_keys($room_coords);
        shuffle($_room_coords);
        foreach($_room_coords as $_coord){
            list($v_2, $n_2) = explode('_', $_coord);
            if(count($_coords) < 7){
                if    (($gate['cond'] == 1121) && ($v_1 > $v_2)){ $_coords[$_coord] = true; }
                elseif(($gate['cond'] == 1112) && ($n_1 < $n_2)){ $_coords[$_coord] = true; }
                elseif(($gate['cond'] == 2111) && ($v_1 < $v_2)){ $_coords[$_coord] = true; }
                elseif(($gate['cond'] == 1211) && ($n_1 > $n_2)){ $_coords[$_coord] = true; }
            }
            if(count($room_coords) >= 10){
                break;
            }
        }
        foreach($_room_coords as $_coord){
            ...
            if(! isset($_coords[$_coord])){
                $_coords[$_coord] = true;
            }
        }
        $room_coord = array_rand($_coords);
        ...
        unset($this->gates[$gate_coord_start]);
        if($gate['type'] == $this->type_gate){
            unset($this->gates[$gate_coord]);
        }
