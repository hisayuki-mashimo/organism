...
$this->_l("PAVE: {$strt_params['rgon']->pos['Y']}_{$strt_params['rgon']->pos['X']} >> ");
$this->_lb("{$trgt_params['rgon']->pos['Y']}_{$trgt_params['rgon']->pos['X']}");
                                        ...
                                        if(isset($this->_sect_tiles[$far_Y][$far_X])){
                                            if($this->_sect_tiles[$far_Y][$far_X] == $this->_type_room){
                                                ...
                                                if($far_rgon !== $strt_params['rgon'] && $far_rgon !== $trgt_params['rgon']){
                                                    $ego_aims = $this->_getEgoAims($aim);
                                                    if($far_rgon->tieds[$ego_aims['D']] === 0){
$this->_lb("<o>{$far_Y}_{$far_X}:{$aim}<<EGO<o>");
                                                        throw new Exception($i);
                                                    }
                                                }
                                            }
