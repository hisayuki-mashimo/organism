...
class Labyrinth
{
    ...
    protected function _setRoomTerritory($room)
    {
        ...
        for($_Y = $edges['T']; $_Y <= $edges['B']; $_Y ++){
            for($_X = $edges['L']; $_X <= $edges['R']; $_X ++){
                ...
                foreach(range($edges[$aim_R], $edges[$aim_L]) as $scdy){
                    ...
                    if($agst_rgon = $this->_getRgon($agst_Y, $agst_X)){
                        if($agst_rgon->type == $this->_type_room){
                            if($agst_rgon->room->inited[$agst_aim] == true){
        ...
        foreach($this->_getAims() as $aim){
            ...
            foreach(range($S_1, $S_2) as $S){
                ...
                if($rgon = $this->_getRgon($rgon_Y, $rgon_X)){
                    ...
                    $this->_gate_cands[$room->id][$gate->id] = $gate;
                }
            }
        }
...
$this->_lb("CRA[{$prev_rgon->pos['Y']}_{$prev_rgon->pos['X']}|{$pres_rgon->pos['Y']}_{$pres_rgon->pos['X']}]");
    ...
    {
        if(! isset($strt_rgon->tieds[$trgt_rgon->pos['Y']][$trgt_rgon->pos['X']])){
            ...
            $strt_rgon->tieds[$trgt_rgon->pos['Y']][$trgt_rgon->pos['X']] = 0;
            $trgt_rgon->tieds[$strt_rgon->pos['Y']][$strt_rgon->pos['X']] = 0;
        }
    }
                            ...
                            foreach($params as $i => $param){
                                ...
                                for($n = 0; $n < $param['limit']; $n ++){
                                    ...
                                    if($this->_sect_tiles[$pres_Y][$pres_X] == $this->_type_wall){
                                        $_sect_tiles[$pres_Y][$pres_X] = $this->_type_wall;
                                        ...
                                        if(isset($this->_sect_tiles[$far_Y][$far_X])){
                                            if($this->_sect_tiles[$far_Y][$far_X] == $this->_type_room){
                                                ...
                                                if($far_rgon !== $strt_params['rgon'] && $far_rgon !== $trgt_params['rgon']){
                                                    if(isset($far_rgon->tieds[$pres_rgon->pos['Y']][$pres_rgon->pos['X']])){
                                                        if($far_rgon->tieds[$pres_rgon->pos['Y']][$pres_rgon->pos['X']] == 0){
                                                            throw new Exception($i);
                                                        }
                                                    }
                                                }
...
$this->_lb('M');
        ...
        $strt_params['rgon']->tieds[$trgt_params['rgon']->pos['Y']][$trgt_params['rgon']->pos['X']] = 1;
        $trgt_params['rgon']->tieds[$strt_params['rgon']->pos['Y']][$strt_params['rgon']->pos['X']] = 1;
    }
...
$this->_lb('PRMY', $prmys);
    ...
    {
        $this->_rgon_tiles[$Y][$X] = new stdClass();
        $this->_rgon_tiles[$Y][$X]->pos   = array('Y' => $Y, 'X' => $X);
        $this->_rgon_tiles[$Y][$X]->type  = $this->_type_wall;
        $this->_rgon_tiles[$Y][$X]->room  = null;
        $this->_rgon_tiles[$Y][$X]->tieds = array();
    ...
    protected function _setRoom($rgon, $chain_id)
    {
        ...
        $this->_room_tiles[$room_id] = new stdClass();
        $this->_room_tiles[$room_id]->id         = $room_id;
        $this->_room_tiles[$room_id]->base_rgon  = $rgon;
        $this->_room_tiles[$room_id]->chain      = $chain_id;
        $this->_room_tiles[$room_id]->rgon_edges = array();
        $this->_room_tiles[$room_id]->sect_pdngs = array('T' => 0, 'R' => 0, 'B' => 0, 'L' => 0);
        $this->_room_tiles[$room_id]->inited     = array('T' => false, 'R' => false, 'B' => false, 'L' => false);
        $this->_room_chain[$chain_id] = array($room_id => true);

        $rgon->type = $this->_type_room;
        $rgon->room = $this->_room_tiles[$room_id];

        return $this->_room_tiles[$room_id];
    }
    ...
    protected function _setRoad($rgon, $room)
    {
        ...
        $this->_road_tiles[$road_id] = new stdClass();
        $this->_road_tiles[$road_id]->rgon  = $rgon;
        $this->_road_tiles[$road_id]->rooms = array($room->id => true);
        $this->_road_tiles[$road_id]->roads = array('Y' => null, 'X' => null);
        $this->_road_tiles[$road_id]->gates = array();
        $this->_specifyIntersection($this->_road_tiles[$road_id]);

        $rgon->type = $this->_type_road;
        $rgon->road = $this->_road_tiles[$road_id];

        return $this->_road_tiles[$road_id];
    }
    ...
    protected function _setGate($rgon, $aim)
    {
        ...
        $this->_gate_tiles[$gate_id] = new stdClass();
        $this->_gate_tiles[$gate_id]->id   = $gate_id;
        $this->_gate_tiles[$gate_id]->rgon = $rgon;
        $this->_gate_tiles[$gate_id]->aim  = $aim;

        unset($this->_gate_cands[$rgon->room->id][$gate_id]);
        if(isset($this->_gate_tasks[$rgon->room->id])){
            $this->_gate_tasks[$rgon->room->id] --;
            if($this->_gate_tasks[$rgon->room->id] <= 0){
                unset($this->_gate_tasks[$rgon->room->id]);
            }
        }
        if(isset($this->_gate_cands[$rgon->room->id]) && empty($this->_gate_cands[$rgon->room->id])){
            unset($this->_gate_cands[$rgon->room->id]);
        }

        return $this->_gate_tiles[$gate_id];
    }
