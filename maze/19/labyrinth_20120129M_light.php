...
//$this->_lb('<$>', $ajst_rgon_far->room->inited[$aim_D], '<$>');
        ...
        foreach($this->_getAims() as $aim){
            ...
            foreach(range($S_1, $S_2) as $S){
                ...
                if($rgon = $this->_getRgon($rgon_Y, $rgon_X)){
                    ...
                    $this->_gate_cands[$room->id][$gate->coord] = $gate;
                }
            }
        }
    ...
    {
$this->_lb("TIERGON >>> {$strt_gate->coord} >>> {$trgt_room->base_rgon->coord}");
        $strt_room = $strt_gate->rgon->room;
...
$this->_lb("CRD[{$prev_rgon->coord}|{$pres_rgon->coord}]");
                ...
                case $this->_type_road:
                    ...
                    $pres_road->rooms[$strt_room->id] = true;
...
$this->_lb("CRA[{$prev_rgon->coord}|{$pres_rgon->coord}]");
                    ...
                    foreach($pres_road->rooms as $room_id => $bool){
                ...
                if($rgon = $this->_getRgon($next_Y, $next_X)){
                    if(($next_Y != $prev_rgon->pos['Y']) || ($next_X != $prev_rgon->pos['X'])){
                        ...
                        if(empty($next_cands) || $range < $next_cands['range']){
                            $next_cands = array('range' => $range, 'aims' => array($aim => $next_coord));
                        }
                        elseif($range == $next_cands['range']){
                            $next_cands['aims'][$aim] = $next_coord;
                        }
                    }
                }
            }
...
$this->_lb('T: ', $trgt_coord);
            foreach($strt_params['coords'] as $strt_coord){
                try{
                    list($strt_Y, $strt_X) = $this->_coordToNumbers($strt_coord);
                    list($trgt_Y, $trgt_X) = $this->_coordToNumbers($trgt_coord);
                    $relation = $this->_getRelation($strt_Y, $strt_X, $trgt_Y, $trgt_X);
                            ...
                            foreach($params as $i => $param){
                                ...
                                for($n = 0; $n < $param['limit']; $n ++){
                                    ...
                                    if($this->_sect_tiles[$pres_Y][$pres_X] == $this->_type_wall){
                                        ...
                                        if(isset($strt_road)){ $this->_registerToRoad($strt_road, $pres_Y, $pres_X); }
                                        if(isset($trgt_road)){ $this->_registerToRoad($trgt_road, $pres_Y, $pres_X); }
                                    }
...
$this->_l("*{$zhou_coord}({$i})");
                            ...
                            if(isset($strt_road)){ $this->_registerToRoad($strt_road, $trgt_Y, $trgt_X); }
                            if(isset($trgt_road)){ $this->_registerToRoad($trgt_road, $trgt_Y, $trgt_X); }
...
//$this->_lb("{$aim}>->{$side_sect->coord}");
    ...
    protected function _setRoad($rgon, $room)
    {
        ...
        $this->_road_tiles[$rgon->coord]->rooms = array($room->id => true);
        $this->_road_tiles[$rgon->coord]->roads = array('Y' => null, 'X' => null);
        $this->_road_tiles[$rgon->coord]->gates = array();
        $this->_specifyIntersection($this->_road_tiles[$rgon->coord]);
    ...
    protected function _setGate($rgon, $aim)
    {
        ...
        unset($this->_gate_cands[$rgon->room->id][$coord]);
