...
class Labyrinth
{
    ...
    public $rgon_scales;                // 管区の規模(最小値: 配列('ver' => 2, 'nex' => 2))
    ...
    public function init($params = array())
    {
        ...
        $this->rgon_scales['ver'] = isset($params['rgon_ver']) ? $params['rgon_ver'] : 2;
        $this->rgon_scales['nex'] = isset($params['rgon_nex']) ? $params['rgon_nex'] : 2;
        if($this->rgon_scales['ver'] < 2){ $this->rgon_scales['ver'] = 2; }
        if($this->rgon_scales['nex'] < 2){ $this->rgon_scales['nex'] = 2; }
    ...
    {
        // 管区集合データの構築
        for($v = 0; $v < $this->rgon_scales['ver']; $v ++){
            for($n = 0; $n < $this->rgon_scales['nex']; $n ++){
                $this->_setRgon($v, $n);
            }
        }

        // 区画集合データの構築
        foreach(range(0, (($this->rgon_side * $this->rgon_scales['ver']) - 1)) as $v){
            foreach(range(0, (($this->rgon_side * $this->rgon_scales['nex']) - 1)) as $n){
                $this->_setSect($v, $n);
            }
        }
...
//$this->_lb($room->base_coord , '///' , $room->rgon_edges, '///', $room->sect_pdngs);
        ...
        // 部屋の設定
        $vect_paramses = array(
            array('prmy' => 'ver', 'scdy' => 'nex', 'vects_prmy' => array('T', 'B'), 'vects_scdy' => array('R', 'L')),
            array('prmy' => 'nex', 'scdy' => 'ver', 'vects_prmy' => array('R', 'L'), 'vects_scdy' => array('T', 'B'))
        );
        foreach($vect_paramses as $turn => $params){
            $axis_ver = rand(0, ($this->rgon_scales['ver'] - 1));
            $axis_nex = rand(0, ($this->rgon_scales['nex'] - 1));
            $axis_coord = $this->_numbersToCoord($axis_ver, $axis_nex);
        ...
        // 部屋の連結
        foreach(range(0, ($this->rgon_scales['ver'] * $this->rgon_scales['nex'])) as $i){
...
$this->_lb('++++', $link[$role]['rgon']->coord, $link[$role]['coords'], '++++');
    ...
    public function setStair()
    {
        ...
        $sect = $this->_getSect($sect_ver, $sect_nex);
    ...
    public function export()
    {
        ...
        $ver_max = $this->rgon_side * $this->rgon_scales['ver'] - 1;
        $nex_max = $this->rgon_side * $this->rgon_scales['nex'] - 1;
        for($v =0; $v <= $ver_max; $v ++){
            $line = '';
            for($n = 0; $n <= $nex_max; $n ++){
    ...
    protected function _collectRoomSeeds()
    {
        ...
        // 一対の対角に必ず部屋を設置する。
        list($ver_min, $ver_max) = array(0, ($this->rgon_scales['ver'] - 1));
        list($nex_min, $nex_max) = array(0, ($this->rgon_scales['nex'] - 1));
        $rgon_corner_pairs = array(
            0 => array(0 => $this->_numbersToCoord($ver_min, $nex_min), 1 => $this->_numbersToCoord($ver_max, $nex_max)),
            1 => array(0 => $this->_numbersToCoord($ver_min, $nex_max), 1 => $this->_numbersToCoord($ver_max, $nex_min))
        );
    ...
    {
        list($ver, $nex) = explode('_', $room->base_coord);
        $edges = array('T' => $ver, 'R' => $nex, 'B' => $ver, 'L' => $nex);
    ...
    {
        $room_ver   = $edges['B'] - $edges['T'] + 1;
        $room_nex   = $edges['R'] - $edges['L'] + 1;
        ...
        foreach($aim_params as $aim => $param){
            ...
            for($var = $param['from']; $var <= $param['to']; $var ++){
                ...
                switch(true){
                    ...
                    case ((($aim == 'T') || ($aim == 'B')) && (($room_ver + 1) > $this->room_side_max)):
                    case ((($aim == 'R') || ($aim == 'L')) && (($room_nex + 1) > $this->room_side_max)):
                        continue 3;
                        break;
                }
    ...
    protected function _setRoomMete($room, $rgon_coord, $edge_aim)
    {
        if($room->inited[$edge_aim] == false){
            $edges = $room->rgon_edges;
            $edge_rgon_coords = array();
            switch($edge_aim){
                case 'T': $edge_params = array('coord' => ($edges['T'] . '_%s'), 'from' => $edges['L'], 'to' => $edges['R']); break;
                case 'R': $edge_params = array('coord' => ('%s_' . $edges['R']), 'from' => $edges['T'], 'to' => $edges['B']); break;
                case 'B': $edge_params = array('coord' => ($edges['B'] . '_%s'), 'from' => $edges['R'], 'to' => $edges['L']); break;
                case 'L': $edge_params = array('coord' => ('%s_' . $edges['L']), 'from' => $edges['B'], 'to' => $edges['T']); break;
            }
            foreach(range($edge_params['from'], $edge_params['to']) as $var){
                $edge_rgon_coords[] = sprintf($edge_params['coord'], $var);
            }

            if(in_array($rgon_coord, $edge_rgon_coords)){
                ...
                foreach($edge_rgon_coords as $edge_rgon_coord){
                ...
                $side_aims = array('R', 'L');
                foreach($side_aims as $aim){
$this->_l('[[', $aim, ']]');
                    $aim_D  = $this->_getEgoAim($edge_aim, 'D');
                    $aim_S  = $this->_getEgoAim($edge_aim, $aim);
                    $aim_SD = $this->_getEgoAim($aim_S, 'D');
                    switch($edge_aim){
                        case 'T': list($edge_ver, $edge_nex, $prmy, $scdy) = array($edges[$edge_aim], $edges[$aim_S], 'ver', 'nex'); break;
                        case 'R': list($edge_ver, $edge_nex, $prmy, $scdy) = array($edges[$aim_S], $edges[$edge_aim], 'nex', 'ver'); break;
                        case 'B': list($edge_ver, $edge_nex, $prmy, $scdy) = array($edges[$edge_aim], $edges[$aim_S], 'ver', 'nex'); break;
                        case 'L': list($edge_ver, $edge_nex, $prmy, $scdy) = array($edges[$aim_S], $edges[$edge_aim], 'nex', 'ver'); break;
                    }
                    $crnr_rgon_coord     = $this->_numbersToCoord($edge_ver, $edge_nex);
...
$this->_lb('(', ($ajst_rgon ? $ajst_rgon->type : 'N'), ')');
                    if($ajst_rgon && $ajst_rgon->type == $this->_type_room){
                        $ajst_poses = array();
                        $ajst_edges = $ajst_rgon->room->rgon_edges;
                        $ajst_poses['ver'] = $ajst_rgon->ver;
                        $ajst_poses['nex'] = $ajst_rgon->nex;
$this->_lb($ajst_poses);
                        switch(true){
                            case ($ajst_rgon->room->rgon_edges[$aim_D]  != $ajst_poses[$prmy]):
                            case ($ajst_rgon->room->rgon_edges[$aim_SD] != $ajst_poses[$scdy]):
                            case ($ajst_rgon->room->inited[$aim_D]  == false):
                            case ($ajst_rgon->room->inited[$aim_SD] == false):
$this->_lb('<#>', $ajst_rgon->room->rgon_edges[$aim_D], '|', $ajst_poses[$prmy], '<#>');
$this->_lb('<#>', $ajst_rgon->room->rgon_edges[$aim_SD], '|', $ajst_poses[$scdy], '<#>');
...
$this->_lb('(', ($ajst_rgon_far ? $ajst_rgon_far->type : 'N'), ')');
                    if($ajst_rgon_far && $ajst_rgon_far->type == $this->_type_room){
                        $ajst_far_poses = array();
                        $ajst_far_edges = $ajst_rgon_far->room->rgon_edges;
                        $ajst_far_poses['ver'] = $ajst_rgon_far->ver;
                        $ajst_far_poses['nex'] = $ajst_rgon_far->nex;
$this->_lb($ajst_far_poses);
                        switch(true){
                            case ($ajst_rgon_far->room->rgon_edges[$aim_D]  != $ajst_far_poses[$prmy]):
                            case ($ajst_rgon_far->room->inited[$aim_D] == false):
$this->_lb('<#>', $ajst_rgon_far->room->rgon_edges[$aim_D], '|', $ajst_far_poses[$prmy], '<#>');
...
if(empty($points_disp)){ vd($room_cands_class, $this->_gate_tasks); }
    ...
    protected function _measureByGateRadar($base_gate, $trgt_room)
    {
        ...
        switch($base_gate->aim){
            case 'T': return ($relation['ver'] < 0) ? (($relation['nex'] == 0) ? 10 : 5) : (($relation['nex'] == 0) ? 1 : 2); break;
            case 'R': return ($relation['nex'] > 0) ? (($relation['ver'] == 0) ? 10 : 5) : (($relation['ver'] == 0) ? 1 : 2); break;
            case 'B': return ($relation['ver'] > 0) ? (($relation['nex'] == 0) ? 10 : 5) : (($relation['nex'] == 0) ? 1 : 2); break;
            case 'L': return ($relation['nex'] < 0) ? (($relation['ver'] == 0) ? 10 : 5) : (($relation['ver'] == 0) ? 1 : 2); break;
        }
    }
...
//$this->_lb('<*>', $trgt_room->base_coord);
            ...
            foreach($this->_getAims() as $aim){
                ...
                if($rgon = $this->_getRgon($coord)){
                    if($coord != $prev_coord){
                        ...
                        $range = abs($relation['ver']) + abs($relation['nex']);
    ...
    protected function _connectRgon($strt_rgon, $trgt_rgon)
    {
        if(! isset($strt_rgon->tieds[$trgt_rgon->coord])){
            ...
            switch(true){
                case ($relation['ver'] != 0): $relation_aim = ($relation['ver'] > 0) ? 'B' : 'T'; break;
                case ($relation['nex'] != 0): $relation_aim = ($relation['nex'] > 0) ? 'R' : 'L'; break;
            }
    ...
    {
        $mdl_ver = ($this->rgon_side * $road->rgon->ver) + floor($this->rgon_side / 2);
        $mdl_nex = ($this->rgon_side * $road->rgon->nex) + floor($this->rgon_side / 2);

        $road->inter = $this->_numbersToCoord($mdl_ver, $mdl_nex);
        $road->roads['ver'] = array($mdl_ver => array($mdl_nex => true));
        $road->roads['nex'] = array($mdl_nex => array($mdl_ver => true));

        $mdl_sect = $this->_getSect($mdl_ver, $mdl_nex);
...
$this->_lb(' REL', $relation);
                    switch($strt_params['aim']){
                        case 'T':
                        case 'B':
                            $range_prmy = abs($relation['ver']);
                            $range_scdy = abs($relation['nex']);
                            $turn_aim   = ($relation['nex'] > 0) ? 'R' : 'L';
                            break;
                        case 'R':
                        case 'L':
                            $range_prmy = abs($relation['nex']);
                            $range_scdy = abs($relation['ver']);
                            $turn_aim   = ($relation['ver'] > 0) ? 'B' : 'T';
                    }
...
//$this->_ll();
    ...
    {
        switch($trgt_aim){
            case 'T': list($prmy, $scdy, $replacer, $sort_prmy, $sort_scdy) = array('ver', 'nex', 'P_S', 'ASC', 'ASC'); break;
            case 'R': list($prmy, $scdy, $replacer, $sort_prmy, $sort_scdy) = array('nex', 'ver', 'S_P', 'DSC', 'ASC'); break;
            case 'B': list($prmy, $scdy, $replacer, $sort_prmy, $sort_scdy) = array('ver', 'nex', 'P_S', 'DSC', 'DSC'); break;
            case 'L': list($prmy, $scdy, $replacer, $sort_prmy, $sort_scdy) = array('nex', 'ver', 'S_P', 'ASC', 'DSC'); break;
        }
$this->_lb('INJUCT >>>> ' . $road->rgon->coord . '|' . $trgt_aim, ' ROAD >>', $road->roads);
        $road_sects = $road->roads[$scdy];
        $scdys      = array_keys($road_sects);
        $ego_rgt    = $this->_getEgoAim($trgt_aim, 'R');
        $ego_lft    = $this->_getEgoAim($trgt_aim, 'L');
...
$this->_lb('PRMY', $prmys);
            ...
            foreach($prmys as $p){
                $coord = str_replace('P', $p, $replacer);
                $coord = str_replace('S', $s, $coord);
//$this->_lb($coord . '**');
                $sect = $this->_getSect($coord);
                if($sect->rgon === $road->rgon){
                    $coord_cands[] = $coord;
                    foreach($side_exists as $aim => $flag){
                        if($flag == 0){
                            $ego_aim = $this->_getEgoAim($trgt_aim, $aim);
                            $side_sect = $this->_getSect($this->_slideCoord($coord, $ego_aim));
                            if($side_sect){
...
//$this->_lb(']]');
    ...
    {
        $sect_ver = $this->rgon_side * $rgon->ver;
        $sect_nex = $this->rgon_side * $rgon->nex;
        $edges = $rgon->room->rgon_edges;
        switch($trgt_aim){
            case 'T':
                $replacer = 'P_S';
                $scdy_min = $sect_nex + (($edges['L'] == $rgon->nex) ? $rgon->room->sect_pdngs['L'] : 0) + 1;
                $scdy_max = $sect_nex - (($edges['R'] == $rgon->nex) ? $rgon->room->sect_pdngs['R'] : 0) - 2 + $this->rgon_side;
                $prmy = $sect_ver + $rgon->room->sect_pdngs['T'];
                break;
            case 'R':
                $replacer = 'S_P';
                $scdy_min = $sect_ver + (($edges['T'] == $rgon->ver) ? $rgon->room->sect_pdngs['T'] : 0) + 1;
                $scdy_max = $sect_ver - (($edges['B'] == $rgon->ver) ? $rgon->room->sect_pdngs['B'] : 0) - 2 + $this->rgon_side;
                $prmy = $sect_nex - $rgon->room->sect_pdngs['R'] - 1 + $this->rgon_side;
                break;
            case 'B':
                $replacer = 'P_S';
                $scdy_min = $sect_nex + (($edges['L'] == $rgon->nex) ? $rgon->room->sect_pdngs['L'] : 0) + 1;
                $scdy_max = $sect_nex - (($edges['R'] == $rgon->nex) ? $rgon->room->sect_pdngs['R'] : 0) - 2 + $this->rgon_side;
                $prmy = $sect_ver - $rgon->room->sect_pdngs['B'] - 1 + $this->rgon_side;
                break;
            case 'L':
                $replacer = 'S_P';
                $scdy_min = $sect_ver + (($edges['T'] == $rgon->ver) ? $rgon->room->sect_pdngs['T'] : 0) + 1;
                $scdy_max = $sect_ver - (($edges['B'] == $rgon->ver) ? $rgon->room->sect_pdngs['B'] : 0) - 2 + $this->rgon_side;
                $prmy = $sect_nex + $rgon->room->sect_pdngs['L'];
                break;
        }
        $replacer = str_replace('P', $prmy, $replacer);
        foreach(range($scdy_min, $scdy_max) as $scdy){
            $coord_cands[] = str_replace('S', $scdy, $replacer);
        }
...
//$this->_lb('____>>', $sect_coord);
        ...
        if(! isset($road->roads['ver'][$ver])){ $road->roads['ver'][$ver] = array(); }
        if(! isset($road->roads['nex'][$nex])){ $road->roads['nex'][$nex] = array(); }
        $road->roads['ver'][$ver][$nex] = true;
        $road->roads['nex'][$nex][$ver] = true;
    }
...
//$this->_lb("@{$road_sect_coord}");
     ...
     * @return
     */
    protected function _setSect($ver, $nex)
    {
        $coord= $this->_numbersToCoord($ver, $nex);
        $rgon = $this->_getRgon(
            floor($ver / $this->rgon_side),
            floor($nex / $this->rgon_side)
        );

        $this->_sect_tiles[$coord] = new stdClass();
        $this->_sect_tiles[$coord]->ver   = $ver;
        $this->_sect_tiles[$coord]->nex   = $nex;
     ...
     * @return
     */
    protected function _setRgon($ver, $nex)
    {
        $coord = $this->_numbersToCoord($ver, $nex);

        $this->_rgon_tiles[$coord] = new stdClass();
        $this->_rgon_tiles[$coord]->ver   = $ver;
        $this->_rgon_tiles[$coord]->nex   = $nex;
    ...
    protected function _setRoad($rgon, $room)
    {
        ...
        $this->_road_tiles[$rgon->coord]->roads = array('ver' => null, 'nex' => null);
     ...
     * @return  array
     */
    /*protected function _getRgonEdges($rgon)
    {
        return array(
            'T' => $this->rgon_side * $rgon->ver,
            'R' => ($this->rgon_side + 1) * $rgon->nex - 1,
            'B' => ($this->rgon_side + 1) * $rgon->nex - 1,
            'L' => $this->rgon_side * $rgon->nex
        );
    }*/
     ...
     * @return  string
     */
    protected function _numbersToCoord($ver, $nex)
    {
        return $ver . '_' . $nex;
    }
    ...
    protected function _slideCoord($coord, $aim, $range = 1)
    {
        ...
        switch($aim){
            case 'T': $ver -= $range; break;
            case 'R': $nex += $range; break;
            case 'B': $ver += $range; break;
            case 'L': $nex -= $range; break;
        }

        return $this->_numbersToCoord($ver, $nex);
    }
    ...
    protected function _getCoordRelation($base_coord, $trgt_coord)
    {
        ...
        return array(
            'ver' => $trgt_ver - $base_ver,
            'nex' => $trgt_nex - $base_nex
        );
    }
}
