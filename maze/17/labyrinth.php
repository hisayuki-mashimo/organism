...
class Labyrinth
{
    ...
    protected function _specifyRoadCoord($pres_coord, $trgt_coord)
    {
        ...
        foreach($this->_getAims() as $aim){
            ...
            switch(true){
                case (! isset($this->_sect_tiles[$zhou_coord])):
                case (isset($road_sects_ver[$ver][$nex]) && ($this->_sect_tiles[$zhou_coord]['aim'] == $aim)):
                    continue;
            }
            ...
            foreach($this->_getAims() as $zhou_aim){
                $adjt_coord = $this->_getCoord($zhou_coord, $zhou_aim);
                $sltg_coord = $this->_getCoord($adjt_coord, $this->_getAgainstAim($zhou_aim, 'rgt'));
