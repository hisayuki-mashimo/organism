    ...
    protected function _specifyGateCands($rgon, $trgt_aim)
    {
        ...
        switch($rgon->type){
            ...
            case $this->_type_road:
                foreach(range($S_R, $S_L) as $S){
                    $pos = array($ego_envs['P'] => $P, $ego_envs['S'] => $S);
                    list($back_Y, $back_X) = $this->_slide($pos['Y'], $pos['X'], $ego_aims['D']);
                    if($this->_sect_tiles[$back_Y][$back_X] == $this->_type_road){
                        $pos_cands = array(array('Y' => $pos['Y'], 'X' => $pos['X']));
                        break;
                    }
                    $pos_cands[] = array('Y' => $pos['Y'], 'X' => $pos['X']);
                }
//vd($rgon->pos, $pos_cands);
                break;
        }
