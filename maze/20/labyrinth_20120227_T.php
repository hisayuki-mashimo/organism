...
class Labyrinth
{
    ...
    protected function _tieRgon($strt_gate, $trgt_room)
    {
        ...
        foreach(range(0, ($this->rgon_scales['Y'] * $this->rgon_scales['X'])) as $i){
            ...
            $links[$link_number][] = array(
                array('rgon' => $prev_rgon, 'aim' => $ego_aims['F']),
                array('rgon' => $pres_rgon, 'aim' => $ego_aims['D'])
            );

            if($pres_rgon->type == $this->_type_room){
                if($prev_room){
                    if($pres_rgon->room !== $prev_room){
                        $prev_room = $pres_rgon->room;
                        $link_number ++;
                    }
                    $links[$link_number] = array();
                }
        ...
        foreach($links as $link_piece){
            foreach($link_piece as $link){
                ...
                foreach($link as $param){
                    if($param['rgon']->type == $this->_type_wall){
                        $this->_setRoad($param['rgon'], $strt_room);
                    }
                    if($param['rgon']->type == $this->_type_road){
                        $param['rgon']->road->gates[$param['aim']] = false;
                        $param['rgon']->road->rooms[] = $strt_room;
                    }
                }
            }
        }
    ...
    {
$this->_lb("PAVE >> {$strt_rgon->pos['Y']}_{$strt_rgon->pos['X']} | {$trgt_rgon->pos['Y']}_{$trgt_rgon->pos['X']}");
...
$this->_lb("PAVEG >> {$strt_pos['Y']}_{$strt_pos['X']} | {$trgt_pos['Y']}_{$trgt_pos['X']}");
        ...
        $turn_prmy_min = ($strt_rgon->type == $this->_type_room) ? 2 : 1;
        $turn_prmy_max = $range_prmy - (($trgt_rgon->type == $this->_type_room) ? 2 : 1);
...
//$this->_lb("PAVED >> {$trgt_params['rgon']->pos['Y']}_{$trgt_params['rgon']->pos['X']}:{$trgt_params['aim']}");
    ...
    protected function _specifyGateCands($rgon, $trgt_aim)
    {
        ...
        switch($rgon->type){
            case $this->_type_room:
                ...
                break;
            case $this->_type_road:
                $this->_specifyIntersection($rgon->road);
                $P   = $rgon->road->edges[$ego_aims['F']];
                $S_R = $rgon->road->edges[$ego_aims['R']];
                $S_L = $rgon->road->edges[$ego_aims['L']];
    ...
    {
        switch($road->axis){
            case 'Y': list($aims_P, $aims_S) = array(array('T', 'B'), array('R', 'L')); break;
            case 'X': list($aims_P, $aims_S) = array(array('R', 'L'), array('T', 'B')); break;
        }
        $inters = array(
            array('Y' => $road->edges['T'], 'X' => $road->edges['R']),
            array('Y' => $road->edges['B'], 'X' => $road->edges['L'])
        );
        shuffle($aims_P);

        // 中心ゲートが行き止まりの状態になっている場合、そこ迄の一本道を削除する。
        foreach($inters as $pos){
            list($road_sect_Y, $road_sect_X) = array($pos['Y'], $pos['X']);
        ...
        $inter_center = ceil(($road->edges[$aims_S[0]] + $road->edges[$aims_S[1]]) / 2);
        $sides = array(
            $this->_getRgonEdge($road->rgon, $aims_S[0]),
            $this->_getRgonEdge($road->rgon, $aims_S[1])
        );
        list($side_min, $side_max) = array(min($sides), max($sides));

        foreach($aims_P as $aim){
            $ego_aims = $this->_getEgoAims($aim);
            $ego_envs = $this->_getEgoEnvs($aim);
            $P_range = range($this->_getRgonEdge($road->rgon, $ego_aims['F']), $this->_getRgonEdge($road->rgon, $ego_aims['D']));
            /*switch($aim){
                case 'T': $P_pre_inter = $road->edges[$aim] - 1; break;
                case 'R': $P_pre_inter = $road->edges[$aim] + 1; break;
                case 'B': $P_pre_inter = $road->edges[$aim] + 1; break;
                case 'L': $P_pre_inter = $road->edges[$aim] - 1; break;
            }
            $P_range = range($this->_getRgonEdge($road->rgon, $ego_aims['F']), $P_pre_inter);*/
            ...
            for($range = 0; $range < ($this->rgon_side / 2); $range ++){
                foreach(array(-1, 1) as $operator){
                    $S = $inter_center + ($range * $operator);
                    if($S < $side_min || $S > $side_max){
                        continue;
                    }

                    foreach($P_range as $P){
                        $pos = array($ego_envs['P'] => $P, $ego_envs['S'] => $S);
$this->_lb("{$pos['Y']}_{$pos['X']}");
    ...
    {
        if(empty($road->edges)){
$this->_lb('POS >> ', $road->rgon->pos);
            $mete_terms = array();
            foreach($this->_getAims() as $aim){
$this->_l("[{$aim}]");
                $ego_aims = $this->_getEgoAims($aim);
                $mete_terms[$aim] = 1;
                list($agst_Y, $agst_X) = $this->_slide($road->rgon->pos['Y'], $road->rgon->pos['X'], $aim);
                if($agst_rgon = $this->_getRgon($agst_Y, $agst_X)){
$this->_l('&');
                    if($agst_rgon->type == $this->_type_room){
$this->_l('*(', $agst_rgon->room->sect_edges, ')');
                        if($agst_rgon->room->sect_pdngs[$ego_aims['D']] < 1){
$this->_l('+');
                            $mete_terms[$aim] = 2;
                        }
                    }
...
$this->_lb('_');
            ...
            $vect_terms = array(
                'Y' => $this->rgon_side - ($mete_terms['T'] + $mete_terms['B']),
                'X' => $this->rgon_side - ($mete_terms['R'] + $mete_terms['L'])
            );
            switch(true){
                case ($vect_terms['Y'] < 2): list($axis, $extd) = array('Y', 'X'); break;
                case ($vect_terms['X'] < 2): list($axis, $extd) = array('X', 'Y'); break;
                default:
                    $vectors = array('Y', 'X');
                    shuffle($vectors);
                    list($axis, $extd) = array($vectors[0], $vectors[1]);
                    break;
            }
            $vector_poses = array(
                'Y' => array('min' => 'T', 'max' => 'B'),
...
$this->_lb('TERMS1 >> ', $mete_terms);
$this->_lb('TERMS2 >> ', $vect_terms);
$this->_lb('AX >> ', $axis);
$this->_lb('EX >> ', $extd);
            $range_extd = rand(2, $vect_terms[$extd]);
            $pos_extd_min_cand_min = $this->_getRgonEdge($road->rgon, $vector_poses[$extd]['min'], $mete_terms[$vector_poses[$extd]['min']]);
            $pos_extd_min_cand_max = $this->_getRgonEdge($road->rgon, $vector_poses[$extd]['max'], $mete_terms[$vector_poses[$extd]['max']] + $range_extd);
            $pos_extd_min = rand($pos_extd_min_cand_min, $pos_extd_min_cand_max);
            $pos_axis = $this->_getRgonEdge($road->rgon, $vector_poses[$axis]['min'], $mete_terms[$vector_poses[$axis]['min']]);
$this->_lb('RANGE_EXTED >> ', $range_extd);
$this->_lb('POS_EXTED_MIN >> ', $pos_extd_min);
$this->_lb('POS_AXIS >> ', $pos_axis);
            foreach(range(0, $range_extd - 1) as $range){
                $pos = array($axis => $pos_axis, $extd => $pos_extd_min + $range);
$this->_lb('SECT >> ', "{$pos['Y']}_{$pos['X']}");
                $this->_sect_tiles[$pos['Y']][$pos['X']] = $this->_type_road;
            }
            $road->edges = array(
                $vector_poses[$axis]['min'] => $pos_axis,
                $vector_poses[$axis]['max'] => $pos_axis,
                $vector_poses[$extd]['min'] => $pos_extd_min,
                $vector_poses[$extd]['max'] => $pos_extd_min + ($range_extd - 1)
            );
            /*$road->inter = array(
                array($axis => $pos_axis, $extd => $pos_extd_min),
                array($axis => $pos_axis, $extd => $pos_extd_min + ($range_extd - 1))
            );*/
            $road->axis = $axis;
        }
    }
