                        ...
                        foreach(range(0, $this->rgon_scales[$env_params['S']]) as $range_S){
                            ...
                            foreach($env_params['aims_S'] as $aim_S_phrase => $ego_aim_S){
                                ...
                                if($rgon = $this->_getRgon($slid_Y, $slid_X)){
                                    ...
                                    switch($phase_S){
                                        case 0:
                                            ($room = $rgon->room) ? $this->_setRoomMete($room, $rgon, $ego_aim_PB) : null;
                                            if(($env_phase > 0) && ($range_P > 0)){
                                                ($rgon->tieds[$ego_aim_PB] === false) ? $this->_paveRoad($rgon, $ego_aim_PB) : null;
                                            }
                                            break;
                                        case 1:
                                            ($room = $rgon->room) ? $this->_setRoomMete($room, $rgon, $ego_aim_P) : null;
                                            if(($env_phase > 0) && ($range_S > 0)){
                                                ($rgon->tieds[$ego_aim_SB] === false) ? $this->_paveRoad($rgon, $ego_aim_SB) : null;
                                            }
                                            break;
                                    }
                                }
...
$this->_lb("##[{$rgon->pos['Y']}_{$rgon->pos['X']}::{$edge_aim}]");
            ...
            if($rgon->pos[$ego_envs['P']] == $edges[$edge_aim]){
                ...
                foreach(array($ego_aims['R'], $ego_aims['L']) as $aim_E){
                    ...
                    if($rgon_T = $this->_slideRgon($rgon_E, $ego_aims['T'])){
                        if($rgon_C = $this->_slideRgon($rgon_T, $ego_aims_E['T'])){
...
$this->_lb("RGON_E[{$rgon_E->pos['Y']}_{$rgon_E->pos['X']}::{$edge_aim}]-<>-");
                                foreach(array($ego_aims['R'], $ego_aims['L']) as $aim_EE){
                                    if($rgon_T->tieds[$aim_EE]){
$this->_lb("AIM_EE[{$aim_EE}]");
                                        $rgon_TE = $this->_slideRgon($rgon_T, $aim_EE);
                                        $ego_aims_EE = $this->_getEgoAims($aim_EE);
                                        $pos_EB = $rgon_TE->tieds[$ego_aims_EE['B']];
                                        if(($sect_edge = $room->sect_edges[$ego_aims_EE['B']]) !== null){
                                            $cond_A = (($ego_envs_E['sort'] == 'ASC') && ($pos_EB['P'] > $sect_edge - 3));
                                            $cond_B = (($ego_envs_E['sort'] == 'DSC') && ($pos_EB['P'] < $sect_edge + 3));
                                            if($cond_A || $cond_B){
$this->_lb('COND >> ', ($cond_A ? 'A' : '') . ($cond_B ? 'B' : ''));
$this->_lb("METE_N::[{$rgon->pos['Y']}_{$rgon->pos['X']}::{$edge_aim}]---[{$rgon_TE->pos['Y']}_{$rgon_TE->pos['X']}::{$ego_aims_EE['B']}] >> ", $pos_EB);
                                                $mete_terms['min'][] = 4 - abs($this->_getRgonEdge($rgon_E, $edge_aim) - $pos_EB['S']);
                                            }
                                        }
                                    }
                                }
                    ...
                    if(($pos_N = $rgon_E->tieds[$aim_E]) === false){
...
$this->_lb("RGON_N[{$rgon_N->pos['Y']}_{$rgon_N->pos['X']}({$aim_E})]");
                            ...
                            if($pos_NB){
                                $pos_NT = $rgon_NB->tieds[$ego_aims['T']];
                                if(($sect_edge = $room->sect_edges[$aim_E]) !== null){
                                    if(abs($pos_NT['S'] - $sect_edge) < 4){
$this->_lb("METE_NT::[{$rgon->pos['Y']}_{$rgon->pos['X']}::{$edge_aim}]---[{$rgon_NB->pos['Y']}_{$rgon_NB->pos['X']}::{$ego_aims['T']}] >> ", $pos_NT);
                                        $mete_terms['max'][] = abs($this->_getRgonEdge($rgon_E, $edge_aim) - $pos_NT['P']) - 3;
                                    }
                                }
                            }
...
$this->_lb("POS_C >> ", $pos_C);
                            if(abs($pos_C['S'] - $this->_getRgonEdge($rgon, $aim_E)) < 4){
$this->_lb('[H]', $this->_getRgonEdge($rgon, $edge_aim), '|', $pos_C['P']);
