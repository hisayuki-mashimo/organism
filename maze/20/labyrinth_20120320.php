        ...
        if($room->sect_edges[$edge_aim] === null){
            ...
            if($rgon->pos[$ego_envs['P']] == $edges[$edge_aim]){
                ...
                foreach($side_aims as $aims){
                    ...
                    list($BSS_Y, $BSS_X) = $this->_slide($BS_Y, $BS_X, $ego_aims[$aims[0]]);
                    switch(true){
                    ...
                    switch(true){
                        ...
                        case (($BSS_rgon = $this->_getRgon($BSS_Y, $BSS_X)) == null):
                        case ($BS_rgon->tieds[$ego_aims['T']]      == null):
                        case ($BS_rgon->tieds[$ego_aims[$aims[0]]] == null):
                        case (($BSS_room = $BSS_rgon->room) == null):
                        case ($BSS_room->sect_pdngs[$ego_aims[$aims[0]]] == null):
                            break;
                        default:
...
$this->_lb();
                    ...
                    if($rgon_T = $this->_slideRgon($rgon_E, $edge_aim)){
                        foreach(array($ego_aims['R'], $ego_aims['L']) as $aim_TE){
                            $ego_envs_TE = $this->_getEgoEnvs($aim_TE);
                            if($pos_TE = $rgon_T->tieds[$aim_TE]){
                                $rgon_TE = $this->_slideRgon($rgon_T, $aim_TE);
                                $pos_TE = $rgon_TE->tieds[$this->_getEgoAims($aim_TE, 'B')];
                                if(abs($pos_TE['S'] - $this->_getRgonEdge($rgon_E, $edge_aim)) < 4){
$this->_l("POS_TE[{$rgon_TE->pos['Y']}_{$rgon_TE->pos['X']}::", $this->_getEgoAims($aim_TE, 'B'), "]---", $pos_TE);
                                    switch(true){
                                        case (($ego_envs_TE['sort'] == 'ASC') && ($room->sect_edges[$this->_getEgoAims($aim_TE, 'B')] - $pos_TE['P'] <  3)):
                                        case (($ego_envs_TE['sort'] == 'DSC') && ($room->sect_edges[$this->_getEgoAims($aim_TE, 'B')] - $pos_TE['P'] > -3)):
$this->_l(':', $room->sect_edges[$this->_getEgoAims($aim_TE, 'B')], ':', $pos_TE['S']);
