...
Labyrinth.prototype = {
    ...
    var rgon_scales:        new Number(2),  // 管区の規模(最小値: 配列('Y' => 2, 'X' => 2))
    var rgon_side:          new Number(2),  // 管区の一辺(最小値: 6)
    var room_count:         new Number(2),  // 部屋の数(最小値: 2)
    var room_side_max:      new Number(2),  // 部屋の一辺の最大値(最小値: 1)
    var room_area_disp:     new Array(),    // 部屋の面積の分布(面積 => 設定確率)(最低要素: 配列(1 => 1))
    ...
    var _rgon_tiles:        new Array(),    // 管区集合データ
    var _room_tiles:        new Array(),    // 部屋集合データ
    var _room_chain:        new Array(),    // 部屋連鎖データ
    var _room_capas:        new Array(),    // 部屋残容量群
    var _road_tiles:        new Array(),    // 通路集合データ
    var _gate_tasks:        new Array(),    // 部屋に設置する接点の個数群
    var _gate_cands:        new Array(),    // 部屋が接点を設置できる地点群
    var _sect_tiles:        new Array(),    // 区画集合データ
    var _obje_tiles:        new Array(),    // オブジェクト集合データ
    var _type_wall:         new Number(0),  // タイプ: 壁面
    var _type_road:         new Number(1),  // タイプ: 通路
    var _type_room:         new Number(2),  // タイプ: 部屋
    var _type_stir_next:    new Number(3),  // タイプ: 階段下り
    var _type_stir_prev:    new Number(4),  // タイプ: 階段上り
    ...
    {
        this.rgon_scales = new Object();
        this.rgon_scales.Y = params.region_y;
        this.rgon_scales.X = params.region_x;

        this.rgon_side = params.unit_size;

        this.room_count = (params.room_count != undefined) ? params.room_count : 2;

        this.room_side_max  = (params.room_side_max != undefined)  ? params.room_side_max : 1;
        this.room_area_disp = (params.room_area_disp != undefined) ? params.room_area_disp : {1: 1};
    },
    ...
    {
        // 管区集合データの構築
        for(var Y = 0; Y < this.rgon_scales.Y; Y ++){
            this._rgon_tiles.Y = array();
            for(var X = 0; X < this.rgon_scales.X; X ++){
                this._rgon_tiles[Y][X] = this._makeRgon(Y, X);
            }
        }

        // 区画集合データの構築
        var Y_max = this.rgon_side * this.rgon_scales.Y;
        var X_max = this.rgon_side * this.rgon_scales.X;
        for(var Y = 0; Y <= Y_max; Y ++){
            this._sect_tiles[Y] = new Array();
        ...
        // 部屋の定義
        var room_seeds = this._collectRoomSeeds();
        for(var room_chain_id in room_seeds){
            var pos = room_seeds[room_chain_id];
            var rgon = this._getRgon(pos['Y'], pos['X']);
...
this._lb("##[{rgon.pos['Y']}_{rgon.pos['X']}::{edge_aim}]");
...
this._lb("RGON_E[{rgon_E.pos['Y']}_{rgon_E.pos['X']}::{edge_aim}]-<>-");
                                foreach(array(ego_aims['R'], ego_aims['L']) as aim_EE){
                                    if(pos_TE = rgon_T.tieds[aim_EE]){
this._lb("AIM_EE[{aim_EE}]");
                                        if((sect_edge = room.sect_edges[this._getEgoAims(aim_EE, 'B')]) !== null){
                                            ...
                                            if(cond_A || cond_B){
this._lb('COND >> ', (cond_A ? 'A' : '') . (cond_B ? 'B' : ''));
this._lb("METE_N::[{rgon.pos['Y']}_{rgon.pos['X']}::{edge_aim}]---[{rgon_T.pos['Y']}_{rgon_T.pos['X']}::{aim_EE}] >> ", pos_TE);
...
this._lb("RGON_N[{rgon_N.pos['Y']}_{rgon_N.pos['X']}({aim_E})]");
                            pos_NB = rgon_N.tieds[ego_aims['B']];
                            if(pos_NB){
                                if((sect_edge = room.sect_edges[aim_E]) !== null){
                                    if(abs(pos_NB['S2'] - sect_edge) < 4){
this._lb("METE_NT::[{rgon.pos['Y']}_{rgon.pos['X']}::{edge_aim}]---[{rgon_NB.pos['Y']}_{rgon_NB.pos['X']}::{ego_aims['T']}] >> ", pos_NB);
...
this._lb("METE_NB::[{rgon.pos['Y']}_{rgon.pos['X']}::{edge_aim}]---[{rgon_NB.pos['Y']}_{rgon_NB.pos['X']}::{ego_aims['T']}] >> ", room_NB.sect_edges);
...
this._lb("RGON_S::[{rgon_S.pos['Y']}_{rgon_S.pos['X']}({aim_E})]");
this._lb("POS_S >> ", pos_S);
                            if(abs(pos_S['S2'] - this._getRgonEdge(rgon, aim_E)) < 4){
this._lb('[H]', this._getRgonEdge(rgon, edge_aim), '|', pos_S['P']);
...
this._lb('<:>', mete_terms);
    ...
    {
this._lb("bb[{rgon.pos['Y']}_{rgon.pos['X']}::{aim}]");
...
if(! isset(poses)){ this._lb('Exception'); this._e(); return; }
    ...
    protected function _setRoadMete(road)
    {
        ...
        if(empty(road.edges)){
            aims = this._getAims();
            ego_aim = aims[array_rand(aims)];
            ego_aims = this._getEgoAims(ego_aim);
            ego_envs = this._getEgoEnvs(ego_aim);
            P_1_min = this._getRgonEdge(road.rgon, ego_aims['T'], 1);
            P_1_max = this._getRgonEdge(road.rgon, ego_aims['B'], 1);
            P_1     = rand(P_1_min, P_1_max);
            P_2_min = this._getRgonEdge(road.rgon, ego_aims['B'], 1);
            P_2_max = P_1;
            P_2     = rand(P_2_min, P_2_max);
            P_length = abs(P_2 - P_1);
            S_min   = this._getRgonEdge(road.rgon, ego_aims['R'], 1);
            S_max   = this._getRgonEdge(road.rgon, ego_aims['L'], 1);
            S       = rand(S_min, S_max);
            road.edges[ego_aims['T']] = P_1;
            road.edges[ego_aims['B']] = P_2;
            road.edges[ego_aims['R']] = S;
            road.edges[ego_aims['L']] = S;
            ego_pos = array(ego_envs['P'] => P_1, ego_envs['S'] => S);
            for(n = 0; n <= P_length; n ++){
                list(Y, X) = this._slide(ego_pos['Y'], ego_pos['X'], ego_aims['B'], n);
                this._sect_tiles[Y][X] = this._type_road;
            }
        }
    }
