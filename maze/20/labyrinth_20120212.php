...
//$this->_lb("CRA[{$prev_rgon->pos['Y']}_{$prev_rgon->pos['X']}|{$pres_rgon->pos['Y']}_{$pres_rgon->pos['X']}]");
    ...
    protected function _paveRoad($strt_params, $trgt_params)
    {
        ...
        foreach(array($strt_params, $trgt_params) as $n => $params){
            if($params['rgon']->type == $this->_type_road){
$this->_lb('WIND >> ', $params['rgon']->pos, '::', $params['aim']);
                $pos = $params['rgon']->pos;
                $ego_aims = $this->_getEgoAims($params['aim']);
                $ego_envs = $this->_getEgoEnvs($params['aim']);
                $aim_Ss = array('R', 'L');
                foreach($aim_Ss as $sn => $aim_S){
                    list($rgon_Y, $rgon_X) = $this->_slide($pos['Y'], $pos['X'], $ego_aims[$aim_S]);
                    if($rgon = $this->_getRgon($rgon_Y, $rgon_X)){
                        switch(true){
                            case ($rgon === $strt_params['rgon']):
                            case ($rgon === $trgt_params['rgon']):
                            case ($rgon->type != $this->_type_room):
                                break;
                            default:
//$this->_lb('SROAD > ', $rgon_Y, '_', $rgon_X);
                                $rgon_ego_aims = $this->_getEgoAims($ego_aims[$aim_Ss[abs($sn - 1)]]);
                                $rgon_ego_envs = $this->_getEgoEnvs($ego_aims[$aim_Ss[abs($sn - 1)]]);
                                $rgon_edges = $rgon->room->rgon_edges;
                                $sect_edges = $rgon->room->sect_edges;
                                $sect_edge_R = ($rgon_edges[$rgon_ego_aims['R']] == $rgon->pos[$rgon_ego_envs['S']]) ? $sect_edges[$rgon_ego_aims['R']] : $rgon->edges[$rgon_ego_aims['R']];
                                $sect_edge_L = ($rgon_edges[$rgon_ego_aims['L']] == $rgon->pos[$rgon_ego_envs['S']]) ? $sect_edges[$rgon_ego_aims['L']] : $rgon->edges[$rgon_ego_aims['L']];
                                $edge_range = abs($sect_edge_R - $sect_edge_L);
                                if($edge_range < 4){
                                    $windmill_cands[$n . '_' . $sn] = array();
                                    $P = $sect_edges[$rgon_ego_aims['F']];
                                    ...
                                    foreach($next_edges as $S){
                                        ...
                                        $windmill_cands[$n . '_' . $sn][] = array('Y' => $Y, 'X' => $X);
$this->_lb('%%', $Y, '_', $X);
                                    }
                                }
                        }
...
$this->_lb("S: {$strt_pos['Y']}_{$strt_pos['X']}");
                    $ego_envs = $this->_getEgoEnvs($strt_params['aim']);
                    ...
                    $turn_prmy_cands_min = ($strt_params['rgon']->type == $this->_type_room) ? 2 : 0;
                    $turn_prmy_cands_max = $range_prmy - (($trgt_params['rgon']->type == $this->_type_room) ? 2 : 0);
...
$this->_lb('M');
        ...
        $strt_params['rgon']->tieds[$strt_params['aim']] = 1;
        $trgt_params['rgon']->tieds[$trgt_params['aim']] = 1;
