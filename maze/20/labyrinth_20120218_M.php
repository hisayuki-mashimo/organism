...
//$this->_lb("CRA[{$prev_rgon->pos['Y']}_{$prev_rgon->pos['X']}|{$pres_rgon->pos['Y']}_{$pres_rgon->pos['X']}]");
    ...
    protected function _paveRoad($strt_rgon, $trgt_rgon)
    {
        ...
        $link = array(
            'strt' => array('rgon' => $strt_rgon, 'aim' => $strt_aim, 'roads' => null),
            'trgt' => array('rgon' => $trgt_rgon, 'aim' => $trgt_aim, 'roads' => null)
        );
        ...
        foreach($link as $role => $param){
            switch($param['rgon']->type){
                ...
                case $this->_type_road:
                    ...
                    $link[$role]['roads'] = $param['rgon']->road->roads;
$this->_lb('WIND >> ', $param['rgon']->pos, '::', $param['aim']);
                    $pos = $param['rgon']->pos;
                    $ego_aims = $this->_getEgoAims($param['aim']);
                    $ego_envs = $this->_getEgoEnvs($param['aim']);
                    $aim_Ss = array('R', 'L');
                    foreach($aim_Ss as $n => $aim_S){
                        list($rgon_Y, $rgon_X) = $this->_slide($pos['Y'], $pos['X'], $ego_aims[$aim_S]);
                        if($rgon = $this->_getRgon($rgon_Y, $rgon_X)){
                            switch(true){
                                case ($rgon === $link['strt']['rgon']):
                                case ($rgon === $link['trgt']['rgon']):
                                case ($rgon->type != $this->_type_room):
                                    break;
                                default:
//$this->_lb('SROAD > ', $rgon_Y, '_', $rgon_X);
                                    $rgon_ego_aims = $this->_getEgoAims($ego_aims[$aim_Ss[abs($n - 1)]]);
                                    $rgon_ego_envs = $this->_getEgoEnvs($ego_aims[$aim_Ss[abs($n - 1)]]);
                                    $rgon_edges = $rgon->room->rgon_edges;
                                    $sect_edge_R = $this->_getRgonEdge($rgon, $rgon_ego_aims['R'], (($rgon_edges[$rgon_ego_aims['R']] == $rgon->pos[$rgon_ego_envs['S']]) ? $rgon->room->sect_pdngs[$rgon_ego_aims['R']] : 0));
                                    $sect_edge_L = $this->_getRgonEdge($rgon, $rgon_ego_aims['L'], (($rgon_edges[$rgon_ego_aims['L']] == $rgon->pos[$rgon_ego_envs['S']]) ? $rgon->room->sect_pdngs[$rgon_ego_aims['L']] : 0));
                                    $edge_range = abs($sect_edge_R - $sect_edge_L);
                                    if($edge_range < 4){
                                        $windmill_cands["{$role}_{$n}"] = array();
                                        $P = $this->_getRgonEdge($rgon, $rgon_ego_aims['F'], $rgon->room->sect_pdngs[$rgon_ego_aims['F']]);
                                        $next_edges = range($sect_edge_R, $sect_edge_L);
                                        if($edge_range > 1){
                                            $next_edges = array_slice($next_edges, 1, (count($next_edges) - 2));
                                        }
                                        foreach($next_edges as $S){
                                            $next_pos = array($rgon_ego_envs['P'] => $P, $rgon_ego_envs['S'] => $S);
                                            list($Y, $X) = $this->_slide($next_pos['Y'], $next_pos['X'], $rgon_ego_aims['F'], 2);
                                            $windmill_cands["{$role}_{$n}"][] = array('Y' => $Y, 'X' => $X);
$this->_lb('%%', $Y, '_', $X);
                                        }
                                    }
                            }
...
$this->_lb("PAVE: {$link['strt']['rgon']->pos['Y']}_{$link['strt']['rgon']->pos['X']} >> {$link['trgt']['rgon']->pos['Y']}_{$link['trgt']['rgon']->pos['X']}");
                            ...
                            foreach($turn_params as $i => $turn_param){
                                ...
                                for($n = 0; $n < $turn_param['limit']; $n ++){
                                    ...
                                    if($this->_sect_tiles[$pres_Y][$pres_X] == $this->_type_wall){
                                        ...
                                        if($link['strt']['rgon']->type == $this->_type_road){ $this->_registerToRoad($link['strt']['rgon']->road, $pres_Y, $pres_X); }
                                        if($link['trgt']['rgon']->type == $this->_type_road){ $this->_registerToRoad($link['trgt']['rgon']->road, $pres_Y, $pres_X); }
                                    }

                                    $zhou_poses = array();
                                    foreach($this->_getAims() as $aim){
                                        $zhou_ego_aims = $this->_getEgoAims($aim);
                                        $zhou_pos_1 = $this->_slide($pres_Y, $pres_X, $aim);
...
$this->_lb("<@>{$pres_Y}_{$pres_X}<@>");
                            ...
                            if($link['strt']['rgon']->type == $this->_type_road){ $this->_registerToRoad($link['strt']['rgon']->road, $trgt_pos['Y'], $trgt_pos['X']); }
                            if($link['trgt']['rgon']->type == $this->_type_road){ $this->_registerToRoad($link['trgt']['rgon']->road, $trgt_pos['Y'], $trgt_pos['X']); }
                            ...
                            if($link['strt']['roads']){ $link['strt']['rgon']->road->roads = $link['strt']['roads']; }
                            if($link['trgt']['roads']){ $link['trgt']['rgon']->road->roads = $link['trgt']['roads']; }
$this->_lb('H');
                        }
                    }
                    if($paved == false){
...
$this->_lb("PAVED >> {$link['strt']['rgon']->pos['Y']}_{$link['strt']['rgon']->pos['X']}:{$link['strt']['aim']}");
