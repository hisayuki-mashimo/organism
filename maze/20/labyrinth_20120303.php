                ...
                if($this->_sect_tiles[$pres_Y][$pres_X] == $this->_type_wall){
                    ...
                    if(! (($i == 0) && ($n == 0))){
                        ...
                        foreach(array('F', 'R', 'D', 'L') as $ego_zhou_aim){
                            ...
                            list($Y_1, $X_1) = $this->_slide($prev_Y, $prev_X, $ego_zhou_aims['F']);
                            ...
                            if($this->_sect_tiles[$Y_1][$X_1] == $this->_type_road){
                                switch(true){
                                    //case ($strt_rgon->type == $this->_type_road && (! isset($strt_rgon->road->roads['Y'][$Y_1][$X_1]))):
                                    //case ($trgt_rgon->type == $this->_type_road && (! isset($trgt_rgon->road->roads['Y'][$Y_1][$X_1]))):
                                    case ($trgt_rgon->type == $this->_type_road && isset($trgt_rgon->road->roads['Y'][$Y_1][$X_1])):
                                        $paved = true;
//vd($strt_rgon->pos['Y'] . '_' . $strt_rgon->pos['X'] . '***' . $Y_1 . '_' . $X_1, $trgt_rgon->road->roads, '-----------------------');
                                        break;
                                }
                            }
                        }
                        ...
                        for($aim_N = 0; $aim_N < 4; $aim_N ++){
                            ...
                            switch($ego_cond){
                                ...
                                case 'OoOiIoOo':
                                case 'OoOoIoOo':
                                    $this->_sect_tiles[$prev_Y][$prev_X] = $this->_type_wall;
                                    break 2;
                            }
                        }
...
$this->_l("{$pos['Y']}_{$pos['X']}::" . implode($conds));
                            for($aim_N = 0; $aim_N < 4; $aim_N ++){
                                ...
                                foreach($patterns[$phase] as $pattern){
                                    if($ego_cond == $pattern){
$this->_l('+');
                                        $this->_sect_tiles[$pos['Y']][$pos['X']] = $this->_type_wall;
                                        break 2;
